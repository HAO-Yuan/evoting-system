...
<body>
	...
	onclick="{window.location.href='<%=basePath%>/voteroperation?action=show'}">view selected Voters</button>
	<button class="btn btn-dark text-right"  type="button"
	onclick="{window.location.href='<%=basePath%>/operationchoice?choice=add'}">Back to Edition</button>
		...
		<c:choose>
		<c:when test="${ not empty voterList}">

			<c:forEach items="${ voterList }" var="voter" varStatus="boucle">
				
					<button class="btn btn-primary btn-lg " style="width: 150px; height: 60px; margin: 20px" type="button"
					onclick="{window.location.href='<%=basePath%>/voteroperation?id=${ voter.id}&action=add'}">${ voter.username}</button>
