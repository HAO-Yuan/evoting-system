...
<body>
	...
	onclick="{window.location.href='<%=basePath%>/candidateoperation?action=show'}">view selected candidates</button>
	<button class="btn btn-dark text-right"  type="button"
	onclick="{window.location.href='<%=basePath%>/operationchoice?choice=setup'}">Back to Setting up</button>
	
		<h1 class = "text-center">Candidates List </h1>
		...
		<c:when test="${ not empty candidatesList}">
			...
			<c:forEach items="${ candidatesList }" var="candidate" varStatus="boucle">
				
					<button class="btn btn-primary btn-lg " style="width: 150px; height: 60px; margin: 20px" type="button"
					onclick="{window.location.href='<%=basePath%>/candidateoperation?id=${ candidate.id}&action=add'}">${ candidate.name}</button>
